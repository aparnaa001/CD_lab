...
void print_row(const char *action) {
    ...
    // Prepare input buffer string (remaining input from ip)
...
// Attempt reduction; return 1 if reduced
...
int main() {
    ...
    while (ip < len) {
        ...
        // Reduce before shifting operators
        if (isOperator(curr)) {
        ...
        while (reduce(next_prec)) {
            // already printed inside reduce
        }
    }

    // Final reduction after input
    int end_prec = 0;
